...
<!--                </FormItem>-->
        ...
        <Table size="small" border stripe :loading="loading" :columns="tableHead" :data="tableData">
            <template slot-scope="{row, index}" slot="recordWay">
                 支付宝支付
            </template>
            <template slot-scope="{row, index}" slot="add_money">
                {{ (+row.add_money).toFixed(2) }}
            </template>
    ...
    export default {
        ...
        methods:{
            async get_data(){
                ...
                let res = await this.Ajax('getBills',{page: this.page})
                if(res){
